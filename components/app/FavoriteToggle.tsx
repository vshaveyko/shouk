...
export function FavoriteToggle({
  ...
  return (
    <button
      type="button"
      onClick={toggle}
      aria-label={favorited ? i18n.t('...app.favoriteToggle.removeFromFavorites') : i18n.t('...app.favoriteToggle.addToFavorites')}
      aria-pressed={favorited}
      data-testid="favorite-toggle"
      className={className}
    >
      <Star size={14} fill={favorited ? "currentColor" : "none"} />
      {favorited ? i18n.t('...app.favoriteToggle.favorited') : i18n.t('...app.favoriteToggle.favorite')}
    </button>
  );
}
